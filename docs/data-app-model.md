...
| `gauge.goal` | `dataset`, `goals` | one ring per goal off the last row; `goals: [{value, goal:{min?, max?}, label?, unit?, format?}]` (each bound a column or a number, at least one — both = a target band); within the band is success-green, under is amber, over is danger-red; hover shows `value (goal …)`; `size: small\|medium\|large` (default medium) sets the shared footprint so several rings share a row (small swaps the badge for a status dot) |
