...
declare a query; an agent runs it over [MCP](/mcp).

```mermaid
flowchart LR
  A[Agent: run_query] --> V{Validate params}
...
- **Agent-authorable.** The spec is plain JSON, so an agent writes a query through the normal
  `replace_manifest` → `apply_edit` loop, the same loop it uses for islands (see
...
of a query as a saved, named, typed version of the ad-hoc `run_sql` MCP tool, scoped to a
single dataset.
...
1. Calls **`list_queries`** to get each declared query: its `name`, `description`, its `params` as
   a JSON Schema, and the result `columns`. That's its grounding for what to pass and what comes
   back.
2. Calls **`run_query({ name, params?, limit? })`**, which validates the params and runs the
...
`replace_manifest` → `apply_edit` loop it uses for islands and actions. No new tool, no file write.
It reads the data, drafts the `queries` block, proposes the full manifest, reviews the diff, and
...
`replace_manifest` and `validate` enforce the same checks: the `dataset` must exist, and every `field`
in `where`, `select`, `groupBy`, and `orderBy` must be a real column on it. A field that doesn't
exist is a named error, caught before the manifest lands: the same fail-loud binding check an
island gets.
...
- [MCP Server](/mcp): `list_queries` and `run_query` inside the agent loop.
