...
- **Agent-authorable.** The spec is plain JSON, so an agent writes a query through the normal
  `propose_edit` → `apply_edit` loop, the same loop it uses for islands (see
...
of a query as a saved, named, typed version of the ad-hoc `query_data` MCP tool, scoped to a
single dataset.
...
`propose_edit` → `apply_edit` loop it uses for islands and actions. No new tool, no file write.
...
`propose_edit` and `validate` enforce the same checks: the `dataset` must exist, and every `field`
