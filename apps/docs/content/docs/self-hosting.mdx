...
- the **MCP server** over [Streamable HTTP](/mcp#running-over-http) at a single `/mcp` — one
  endpoint for the whole workspace, with the app picked by the `app` tool param.
...
the token as a bearer header. One endpoint hosts the whole workspace; the client picks the app with
the `app` tool param (omitted when there's only one), and `list_apps` enumerates them.
...
posture from the [MCP page](/mcp) apply — the transport is the only thing that changes. App ids
never appear in the URL, so the single `/mcp` endpoint serves every app in the workspace.
