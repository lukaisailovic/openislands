...
island's range straight from `get_island_schema` over the MCP. The runtime also floors spans
responsively, so a tile never renders narrower than its usable width on a small screen.

## Fail loudly

The manifest's job is to make a broken dashboard **impossible to ship silently.** Every
island binds to named fields, and `validate` (and the agent's `replace_manifest`) checks each one
