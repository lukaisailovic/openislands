---
title: Tables & Feeds
description: Three islands for rows you read one at a time — a table when exact values matter, a feed for things that happened in order, and a search box to find one row in many.
---
...
- `columns: [{ field, label?, format? }]`: pick and order the columns and set each column's
  [value format](/reference/value-formats). Omit it to show every column.
- `details: [{ field, label?, format? }]`: fields *hidden* from the row and revealed in a
  click-to-open dialog, each with its own optional [value format](/reference/value-formats).
...
- **Rich rows.** Setting any of `highlight`, `stats`, or `footer` switches a row from a single
  ...
  Each of these takes an optional [value format](/reference/value-formats) (`format`) or
