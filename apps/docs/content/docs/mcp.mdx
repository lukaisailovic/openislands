...
| `app.runSql({ dataset } \| { sql }, limit)` | Rows from a dataset, or a read-only `SELECT` over the registered dataset views. |
...
OAuth runs in the dashboard browser, not from the agent. If `listConnectors()` shows a connector
