import type { LiveIslandData } from "./LiveIsland.client.js";

export type { LiveIslandData };

/**
 * Build the `data` prop a `<LiveIsland>` feeds its renderer: the dataset name,
 * a column list (name + DuckDB-style type), and the rows. This is the same
 * shape the runtime hands an island after a query resolves, so islands in the
 * docs behave exactly like islands in a live dashboard.
 */
