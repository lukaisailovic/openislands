import { LiveIsland } from "../../components/LiveIsland.client";
import { netWorthByMonth } from "../../components/sampleData";

# Islands

An **island** is a reusable, typed visual block. You don't write rendering code — you
declare an island in the manifest, bind it to fields that exist in your data, and the
runtime renders it. The same renderers the runtime ships power the live previews below:
each tile is a real OpenIslands island, not a screenshot.

## A metric, live

A `metric.kpi` reads the last row of its dataset for the value, the row before it for the
delta, and draws a sparkline over the time axis it finds.

<LiveIsland
  type="metric.kpi"
  config={{ title: "Net worth", value: "net_worth_eur", compareTo: "prev", format: "eur" }}
  data={netWorthByMonth}
/>

```jsonc
{
  "type": "metric.kpi",
  "title": "Net worth",
  "dataset": "net_worth",
  "value": "net_worth_eur",
  "compareTo": "prev",
  "format": "eur"
}
```

## A note, live

A `note.card` renders Markdown and binds to no data — handy for context, captions, and
links right inside a dashboard.

<LiveIsland
  type="note.card"
  config={{ markdown: "## Hello\nLive island." }}
  height={120}
/>

## The island registry

Every built-in island and the fields it requires. Pick the built-in that fits before
reaching for a custom renderer.

| Type | Required fields | What it is |
| --- | --- | --- |
| `metric.kpi` | `dataset`, `value` | A single number off the last row, with an optional delta and sparkline. |
| `timeseries.line` | `dataset`, `x`, `y` | A line chart over time; `y` may be one field or many. |
| `category.bar` | `dataset`, `x`, `y` | A bar chart, optionally grouped or stacked. |
| `breakdown.treemap` | `dataset`, `label`, `value` | A treemap of parts of a whole. |
| `table.grid` | `dataset` | A data table with optional details, grouping, and drilldown. |
| `timeline.feed` | `dataset`, `ts`, `titleField` | A chronological feed of rows, plain or rich. |
| `gauge.rings` | `dataset`, `rings` | Concentric goal rings off the last row. |
| `gauge.goal` | `dataset`, `value`, `goal` | A single ring measured against a goal or band. |
| `gauge.meter` | `dataset`, `meters` | Horizontal usage bars off the last row. |
| `search.box` | `dataset`, `fields`, `titleField` | A client-side search over a dataset. |
| `note.card` | `markdown` | Markdown content; no data. |
| `source.doc` | — | An embedded document or link (`file` or `href`). |
| `layout.row` | `islands` | A full-width structural row holding other islands. |

See the per-category pages for configuration details: [Metrics & Gauges](/islands/metrics-and-gauges),
[Charts](/islands/charts), [Tables & Feeds](/islands/tables-and-feeds),
[Content & Layout](/islands/content-and-layout), and [Custom Islands](/islands/custom).
