...
Five islands for at-a-glance numbers: a headline KPI, a multi-stat scorecard, and three gauges. Each previewed
...
- `stats[].label` defaults to the field name; `format`/`unit` style the value; `compareTo: "prev"`
  adds a delta badge versus the previous row.
