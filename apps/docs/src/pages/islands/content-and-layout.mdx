...
or link, a **content editor** for a whole workspace of markdown files, and a **layout row** to
control how a page wraps. See the [islands overview](/islands/overview) for the full registry.
...
- `file` / `dir`: the source, and exactly one is required — `file` is one document under
  `data/docs/`, `dir` is a directory (recursed). Both, or neither, is a named validation error.
...
- `csv` (`dir` only, default `false`): also surface `.csv` files alongside the markdown. CSVs open
  **read-only** as a table — the editor edits markdown, not spreadsheets.
...
**Editing and version history.** Unless `readOnly` is set, edits save straight back to the file on
disk, and the file is the source of truth — change it in your own editor and the workspace reflects
it. Every save also records a restorable snapshot in a per-app SQLite store at
...
is a full workspace bound to a `data/` tree with CSV view and the three virtual folders above.
</Callout>
