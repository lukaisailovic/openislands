...
caption, instructions, or a link, dropped right between data islands. It takes only `markdown` —
headings, lists, inline code, bold/italic, and links all render.
...
- `file` — a path relative to the project root; it resolves through the runtime's confined file
  route. `href` — an external URL used as-is.
