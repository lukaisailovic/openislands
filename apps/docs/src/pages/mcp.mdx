...
The CLI is how *you* maintain a dashboard. The MCP server is how an **agent** does — safely,
for months, without it rotting. It exposes the same typed, validated path as a set of tools
over the Model Context Protocol, built on one principle:

> **Read many, write one.** An agent can read everything — the manifest, the schemas, the
> live data — but every change funnels through a single proposal-and-apply pipeline that
> validates before it writes and snapshots before it changes anything.
...
data, and history under it. `npx -y` fetches and runs the latest published server with no
prior install.
...
<Callout variant="warning">
  **Authorization is human-only.** OAuth runs in the dashboard browser, not from the agent. If
  `list_connectors` shows a connector isn't `connected` (OAuth not completed, or secrets
  missing), the agent must tell you to open the dashboard (`openislands serve`) and click
  **Connect** — it must never attempt to authorize on its own.
</Callout>
...
- **Prompt-injection posture.** Because the *only* mutations are a validated full-manifest
  proposal and a schema-checked row insert — both diffed or reported and both reversible —
  data that tries to talk an agent into a harmful edit still can't bypass validation, the
  diff, or the rollback snapshot.
